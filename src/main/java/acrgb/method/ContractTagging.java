        ...
        try {
            switch (tags.trim().toUpperCase()) {
                case "HCI": {
                    ...
                    ACRGBWSResult contractList = fm.GETCONBYCODE(datasource, con.getHcfid().trim());
                    if (contractList.isSuccess()) {
                        ...
                        if (!contract.getContractdate().isEmpty()) {
                            ...
                            ACRGBWSResult updateConANDAssets = updateMethod.CONSTATSUPDATE(datasource, contract.getConid(), contract.getStats(), contract.getRemarks(), contract.getEnddate());
                            if (!updateConANDAssets.isSuccess()) {
                                error.add(updateConANDAssets.getMessage());
                            }
                            //UPDATE ROLE INDEX 
                            ACRGBWSResult updateAccessID = updateMethod.UPDATEMAPPEDROLEBASECONDATE(datasource, con.getHcfid(), conDate.getCondateid(), "HCI");
                ...
                case "HCPN": {
                    ...
                    if (contractList.isSuccess()) {
                        ...
                        if (!contract.getContractdate().isEmpty()) {
                            ...
                            //UPDATE ROLE INDEX 
                            ACRGBWSResult updateAccessID = updateMethod.UPDATEMAPPEDROLEBASECONDATE(datasource, con.getHcfid(), conDate.getCondateid(), "HCPN");
                            if (!updateAccessID.isSuccess()) {
                                error.add(updateAccessID.getMessage());
                            }
                        }
                    ...
                    ACRGBWSResult getRole = methods.GETROLEMULITPLE(datasource, con.getHcfid().trim(), "ACTIVE");
                    if (getRole.isSuccess()) {
                        ...
                        for (int i = 0; i < listRole.size(); i++) {
                            ...
                            if (contractList.isSuccess()) {
                                //MAPPED CONTRACT
                                Contract contract = utility.ObjectMapper().readValue(contractHCIList.getResult(), Contract.class);
                                if (!contract.getContractdate().isEmpty()) {
                                    //MAPPED CONTRACT DATE
                                    ContractDate conDate = utility.ObjectMapper().readValue(contract.getContractdate(), ContractDate.class);
                                    //UPDATE CONTRACT AND ASSETS UNDER 
                                    ACRGBWSResult updateConANDAssets = updateMethod.CONSTATSUPDATE(datasource, contract.getConid().trim(), contract.getStats().trim(), contract.getRemarks(), contract.getEnddate().trim());
                                    if (!updateConANDAssets.isSuccess()) {
                        ...
                        error.add("No Facility Found Under HCPN " + con.getHcfid().trim());
                    }
                    result.setSuccess(true);
                    break;
                }
